...
class WDTTools {
    ...
    private static function wdtIsDateTime($input) {
        return (
            ...
            (
                call_user_func('preg_match', WDT_TIME_12H_REGEX, substr($input, strpos($input, ' ') + 1)) ||
                call_user_func('preg_match', WDT_TIME_24H_REGEX, substr($input, strpos($input, ' ') + 1))
    ...
    public static function enqueueBootstrapJS() {
        ...
        if (!$bootstrapEnqueued) {
            wp_enqueue_script('wdt-bootstrap', WDT_ROOT_URL . 'assets/js/bootstrap/bootstrap.min.js', array(), false, true);
        }
    }
    ...
    public static function wdtConvertStringToUnixTimestamp($dateString, $dateFormat) {
        ...
        } else if (isset($dateFormat) && in_array($dateFormat, ['m.d.Y', 'm-d-Y'])) {
            $returnDate = strtotime(str_replace(['.', '-'], '/', $dateString));
        } else {
...
add_action('wp_enqueue_scripts', array('WDTTools', 'enqueueBootstrapJS'), PHP_INT_MAX);
