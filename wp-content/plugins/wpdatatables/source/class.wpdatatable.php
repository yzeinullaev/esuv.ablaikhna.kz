...
class WPDataTable {
            ...
            foreach ($wdtColumnTypes as $key => $columnType) {
                if (in_array($columnType, array('date', 'datetime', 'time'))) {
                    foreach ($this->_dataRows as &$dataRow) {
                        $dataRow[$key] = WDTTools::wdtConvertStringToUnixTimestamp($dataRow[$key], $wdtParameters['dateInputFormat'][$key]);
                    }
                }
            }
        }
                ...
                if (!empty($sumColumns) || !empty($avgColumns)) {
                    ...
                    if (strpos($query, '*') === false) {
                        foreach ($output['sumAvgColumns'] as $columnTitle) {
                            $sumFunctionQuery = preg_replace("/{$columnTitle}/", 'SUM(`' . $columnTitle . '`) AS `' . $columnTitle . '`', $functionsQuery, 1);
                        }
                    } else {
                        $selectSumArr = array();
                        foreach ($output['sumAvgColumns'] as $columnTitle) {
                ...
                if (!empty($minColumns)) {
                    ...
                    if (strpos($query, '*') === FALSE) {
                        foreach ($output['minColumns'] as $columnTitle) {
                            $minFunctionQuery = preg_replace("/{$columnTitle}/", 'MIN(`' . $columnTitle . '`) AS `' . $columnTitle . '`', $functionsQuery, 1);
                        }
                    } else {
                        $selectMinArr = array();
                        foreach ($output['minColumns'] as $columnTitle) {
                ...
                if (!empty($maxColumns)) {
                    ...
                    if (strpos($query, '*') === FALSE) {
                        foreach ($output['maxColumns'] as $columnTitle) {
                            $maxFunctionQuery = preg_replace("/{$columnTitle}/", 'MAX(`' . $columnTitle . '`) AS `' . $columnTitle . '`', $functionsQuery, 1);
                        }
                    } else {
                        $selectMaxArr = array();
                        foreach ($output['maxColumns'] as $columnTitle) {
                    ...
                    if (!empty($wdtParameters['data_types'][$dataColumnHeading]) && in_array($wdtParameters['data_types'][$dataColumnHeading], array('date', 'datetime', 'time'))) {
                        if ($format === 'xls') {
                            ...
                            if (PHPExcel_Shared_Date::isDateTime($cell)) {
                                ...
                                $namedDataArray[$r][$dataColumnHeading] = WDTTools::wdtConvertStringToUnixTimestamp($dataRow[$row][$dataColumnIndex], $wdtParameters['dateInputFormat'][$dataColumnHeading]);
                            }
                        } elseif ($format === 'csv') {
                            $namedDataArray[$r][$dataColumnHeading] = WDTTools::wdtConvertStringToUnixTimestamp($dataRow[$row][$dataColumnIndex], $wdtParameters['dateInputFormat'][$dataColumnHeading]);
                        }
    ...
    protected function enqueueJSAndStyles() {
        ...
        if (get_option('wdtMinifiedJs')) {
            ...
            wp_enqueue_script('wdt-wpdatatables', WDT_JS_PATH . 'wpdatatables/wdt.frontend.min.js', array(), false, true);
        } else {
    ...
    public static function solveFormula($formula, $headers_in_formula, $row) {
        ...
        foreach ($headers_in_formula as $header) {
            ...
            $vars[WDTTools::sanitizeHeader($header)] = (float)$row[$header];
        };
    ...
    public static function deleteTable($tableId) {
        ...
        if (empty($tableId) || !current_user_can('manage_options')) {
            return false;
        }
